...
%		L'effet Hall a été mis en évidence en 1880 par le physicien du même nom : dans un conducteur traversé par un courant et placé dans un champ magnétique, on observe l'apparition d'une tension transverse. Cette tension caractérisée par Hall est proportionnelle à l'intensité du courant et au champ magnétique, et inversement proportionnelle à la largeur du conducteur considéré. Nous allons chercher à expliquer ce résultat. Pour cela, nous étudierons le système présenté sur la figure ??, les charges considérées étant ici des électrons.\\
%		Nous sommes ici en présence de charges mobiles au sein d'un champ magnétique. Elles subissent donc une force magnétique :
%		\begin{equation*}
%			\vec{F}_{mag} = q \vec{v} \wedge \vec{B}
%		\end{equation*}
%		qui va dévier leur trajectoire. Cette déviation va induire une accumulation de charges négatives d'un côté de l'échantillon et de charges positives de l'autre (due ici à un déficit d'électrons de ce côté). Cela explique l'apparition d'une tension transverse dans le système.\\
%		Cependant, il faut à présent noter que cela implique également l'apparition d'un champ électrique (comme illustré sur la figure) qui va exercer sur les électrons une force $ \vec{F}_{elec} = q \vec{E}_H $ de même direction que $ \vec{F}_{mag} $ mais de sens opposé. Cette force va donc progressivement augmenter jusqu'à égaliser en norme avec la force magnétique : on atteint alors le régime permanent. Nous avons alors :
%		\begin{equation*}
%			q \vec{E}_H + q \vec{v} \wedge \vec{B} = \vec{0} \; .
%		\end{equation*}
%		Ce qui nous amène à l'obtention de l'expression de la tension de Hall :
%		\begin{equation}
%			V_H = E_H \, a = v B a \; .
%		\end{equation}
%		Dans le cas où l'on considère une densité $ n $ de porteurs de charge, la vitesse est liée à l'intensité du courant par :
%		\begin{equation*}
%			v = \frac{I}{n q a b}
%		\end{equation*}
%		où $ b $ est ici l'épaisseur de l'échantillon. Nous trouvons alors l'expression finale pour la tension de Hall :
%		\begin{equation*}
%			V_H = \frac{1}{nq} \frac{I B}{b}
%		\end{equation*}
	\end{encart}
